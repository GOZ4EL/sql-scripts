...
SELECT 
  e.Apellido, 
  e.Oficio AS "Función", 
  d.DNombre AS "Sala o departamento" 
  FROM Emp e
    JOIN Dept d 
	  ON e.Dept_No = d.Dept_No
UNION  
...
FROM Emp e
  JOIN Dept d 
    ON e.Dept_No = d.Dept_No
UNION  
SELECT p.Apellido, p.Funcion, p.Salario, s.Nombre 
  FROM Plantilla p
    JOIN Sala s 
	  ON s.Sala_Cod = p.Sala_Cod;
...
SELECT p.Apellido, p.Funcion, p.Salario, p.Hospital_Cod 
FROM Plantilla p
  JOIN Hospital h 
    ON p.Hospital_Cod = h.Hospital_Cod
WHERE (
  UPPER(p.funcion) LIKE UPPER("Enfermer%")
...
FROM Plantilla p
  JOIN Hospital h 
    ON p.Hospital_Cod = h.Hospital_Cod 
WHERE (
  UPPER(h.Nombre) = UPPER("Provincial")
